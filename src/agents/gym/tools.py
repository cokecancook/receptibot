...
def check_gym_availability(target_date: str) -> str:
    ...
    - If no time is defined, booking_date will include T00:00:00, as YYYY-MM-DDT00:00:00. (Pending! Now time defined is represented as YYYY-MM-DD)
        - API RESPONSE will be the first 3 available slots from YYYY-MM-DDT08:00:00 to YYYY-MM-DDT21:00:00. (Fix Pending! Now next slots are returned even if at full capacity and technically unavailable)
    - If time is defined, booking_date will be as: from YYYY-MM-DDT08:00:00 to YYYY-MM-DDT21:00:00.
        - If time is defined, and slot is available, answer from the API will be the available slot. (Fix Pending! Now target_slot is returned even if at full capacity and technically unavailable)
...
def book_gym_slot(booking_date: str, user_name: str) -> str:
    ...
    - If not available, answer from the API will be a message of unavailability and the next 3 available slots from YYYY-MM-DDT08:00:00 to YYYY-MM-DDT21:00:00. (Pending implementation for returning next 3 available slots)
