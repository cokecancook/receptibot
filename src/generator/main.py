...
def generate_bookings(session, max_fill_rate: float = 0.5, full_week_prob: float = 0.2):
    slots = session.query(Slot).all()
    weeks = defaultdict(list)
    for slot in slots:
        y, w, _ = slot.start_time.isocalendar()
        weeks[(slot.service_id, y, w)].append(slot)

    bookings = []
    for (svc_id, year, week), week_slots in weeks.items():
        if random.random() < full_week_prob:
            fill_rate = 1.0
        else:
    ...
    session.commit()
    print(f"Generadas {len(bookings)} reservas de prueba con realismo semanal.")
